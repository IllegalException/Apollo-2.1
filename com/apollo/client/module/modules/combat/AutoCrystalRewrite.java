...
public class AutoCrystalRewrite extends Module
{
    ...
    public AutoCrystalRewrite() {
        ...
        this.packetSendListener = (Listener<PacketEvent.Send>)new Listener(event -> {
            ...
            if (packet instanceof CPacketPlayer && this.spoofRotations.getValue() && AutoCrystalRewrite.isSpoofingAngles) {
                ((CPacketPlayer)packet).yaw = (float)AutoCrystalRewrite.yaw;
                ((CPacketPlayer)packet).pitch = (float)AutoCrystalRewrite.pitch;
            }
        }, new Predicate[0]);
    ...
    public void setup() {
        ...
        AutoCrystalRewrite.placeRange = this.registerDouble("Place Range", "PlaceRange", 6.0, 0.0, 6.0);
        AutoCrystalRewrite.range = this.registerDouble("Hit Range", "HitRange", 5.0, 0.0, 10.0);
    ...
    public void onUpdate() {
        ...
        this.isBreaking = false;
        this.isPlacing = false;
        if (AutoCrystalRewrite.mc.player == null || AutoCrystalRewrite.mc.player.isDead) {
            return;
        }
        final EntityEnderCrystal crystal = (EntityEnderCrystal)AutoCrystalRewrite.mc.world.loadedEntityList.stream().filter(entity -> entity instanceof EntityEnderCrystal).filter(e -> AutoCrystalRewrite.mc.player.getDistance(e) <= AutoCrystalRewrite.range.getValue()).filter(e -> this.crystalCheck(e)).map(entity -> entity).min(Comparator.comparing(c -> AutoCrystalRewrite.mc.player.getDistance(c))).orElse(null);
        if (this.explode.getValue() && crystal != null) {
            if (!AutoCrystalRewrite.mc.player.canEntityBeSeen((Entity)crystal) && AutoCrystalRewrite.mc.player.getDistance((Entity)crystal) > this.walls.getValue()) {
                return;
            }
            if (this.antiWeakness.getValue() && AutoCrystalRewrite.mc.player.isPotionActive(MobEffects.WEAKNESS)) {
                if (!this.isAttacking) {
                    this.oldSlot = AutoCrystalRewrite.mc.player.inventory.currentItem;
                    this.isAttacking = true;
                }
                this.newSlot = -1;
                for (int i = 0; i < 9; ++i) {
                    final ItemStack stack = AutoCrystalRewrite.mc.player.inventory.getStackInSlot(i);
                ...
                if (this.newSlot != -1) {
                    AutoCrystalRewrite.mc.player.inventory.currentItem = this.newSlot;
            ...
            if (System.nanoTime() / 1000000L - this.breakSystemTime >= 420 - this.attackSpeed.getValue() * 20) {
                ...
                if (this.rotate.getValue()) {
                    this.lookAtPacket(crystal.posX, crystal.posY, crystal.posZ, (EntityPlayer)AutoCrystalRewrite.mc.player);
                }
                AutoCrystalRewrite.mc.playerController.attackEntity((EntityPlayer)AutoCrystalRewrite.mc.player, (Entity)crystal);
                if (this.handBreak.getValue().equalsIgnoreCase("Offhand") && !AutoCrystalRewrite.mc.player.getHeldItemOffhand().isEmpty) {
                    AutoCrystalRewrite.mc.player.swingArm(EnumHand.OFF_HAND);
                }
                else {
            ...
            resetRotation();
            if (this.oldSlot != -1) {
                AutoCrystalRewrite.mc.player.inventory.currentItem = this.oldSlot;
                this.oldSlot = -1;
            }
            this.isAttacking = false;
            this.isActive = false;
        ...
        int crystalSlot = (AutoCrystalRewrite.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) ? AutoCrystalRewrite.mc.player.inventory.currentItem : -1;
        if (crystalSlot == -1) {
            for (int l = 0; l < 9; ++l) {
                if (AutoCrystalRewrite.mc.player.inventory.getStackInSlot(l).getItem() == Items.END_CRYSTAL) {
                    crystalSlot = l;
                    break;
                }
            }
        ...
        entities.addAll((Collection<? extends Entity>)AutoCrystalRewrite.mc.world.playerEntities.stream().filter(entityPlayer -> !Friends.isFriend(entityPlayer.getName())).sorted(Comparator.comparing(e -> AutoCrystalRewrite.mc.player.getDistance(e))).collect(Collectors.toList()));
        BlockPos q = null;
        double damage = 0.5;
        ...
        while (true) {
            if (!var9.hasNext()) {
                ...
                if (this.place.getValue()) {
                    if (!offhand && AutoCrystalRewrite.mc.player.inventory.currentItem != crystalSlot) {
                        if (this.autoSwitch.getValue()) {
                            AutoCrystalRewrite.mc.player.inventory.currentItem = crystalSlot;
                            resetRotation();
                            this.switchCooldown = true;
                        }
                        return;
                    ...
                    if (this.rotate.getValue()) {
                        this.lookAtPacket(q.getX() + 0.5, q.getY() - 0.5, q.getZ() + 0.5, (EntityPlayer)AutoCrystalRewrite.mc.player);
                    }
                    final RayTraceResult result = AutoCrystalRewrite.mc.world.rayTraceBlocks(new Vec3d(AutoCrystalRewrite.mc.player.posX, AutoCrystalRewrite.mc.player.posY + AutoCrystalRewrite.mc.player.getEyeHeight(), AutoCrystalRewrite.mc.player.posZ), new Vec3d(q.getX() + 0.5, q.getY() - 0.5, q.getZ() + 0.5));
                    ...
                    if (q != null && AutoCrystalRewrite.mc.player != null) {
                        this.isActive = true;
                        this.isPlacing = true;
                        if (this.raytrace.getValue() && this.f != null) {
                            AutoCrystalRewrite.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(q, this.f, offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                        }
                        else if (q.getY() == 255) {
                            AutoCrystalRewrite.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(q, EnumFacing.DOWN, offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                        }
                        else {
                            AutoCrystalRewrite.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(q, EnumFacing.UP, offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                        }
                    ...
                    if (AutoCrystalRewrite.isSpoofingAngles) {
                        if (AutoCrystalRewrite.togglePitch) {
                            final EntityPlayerSP var10 = AutoCrystalRewrite.mc.player;
                            var10.rotationPitch += (float)4.0E-4;
                            AutoCrystalRewrite.togglePitch = false;
                        }
                        else {
                            final EntityPlayerSP var10 = AutoCrystalRewrite.mc.player;
                            var10.rotationPitch -= (float)4.0E-4;
                            AutoCrystalRewrite.togglePitch = true;
                        }
                    }
                    return;
                }
            ...
            if (entity2 != AutoCrystalRewrite.mc.player && entity2.getHealth() > 0.0f) {
                for (final BlockPos blockPos : blocks) {
                    ...
                    if (entity2.getDistanceSq(x, y, z) < this.enemyRange.getValue() * this.enemyRange.getValue()) {
                        ...
                        final double self = calculateDamage(blockPos.getX() + 0.5, blockPos.getY() + 1, blockPos.getZ() + 0.5, (Entity)AutoCrystalRewrite.mc.player);
                        if (self >= this.maxSelfDmg.getValue() || self >= AutoCrystalRewrite.mc.player.getHealth() + AutoCrystalRewrite.mc.player.getAbsorptionAmount()) {
                            continue;
                        }
                        damage = d;
                        q = blockPos;
    ...
    public void onWorldRender(final RenderEvent event) {
        ...
        if (this.showDamage.getValue() && this.render != null && this.renderEnt != null) {
            ...
            ApolloTessellator.glBillboardDistanceScaled(this.render.getX() + 0.5f, this.render.getY() + 0.5f, this.render.getZ() + 0.5f, (EntityPlayer)AutoCrystalRewrite.mc.player, 1.0f);
            ...
            GlStateManager.translate(-(AutoCrystalRewrite.mc.fontRenderer.getStringWidth(damageText) / 2.0), 0.0, 0.0);
    ...
    private boolean validTarget(final Entity entity) {
        return entity != null && entity instanceof EntityLivingBase && !Friends.isFriend(entity.getName()) && !entity.isDead && ((EntityLivingBase)entity).getHealth() > 0.0f && entity instanceof EntityPlayer && entity != AutoCrystalRewrite.mc.player;
    }
    
    private EntityLivingBase GetNearTarget(final Entity distanceTarget) {
        return (EntityLivingBase)AutoCrystalRewrite.mc.world.loadedEntityList.stream().filter(entity -> this.validTarget(entity)).map(entity -> entity).min(Comparator.comparing(entity -> distanceTarget.getDistance(entity))).orElse(null);
    }
    
    private boolean rayTraceCheckBreak(final EntityEnderCrystal crystal) {
        return !this.raytrace.getValue() || AutoCrystalRewrite.mc.player.canEntityBeSeen((Entity)crystal);
    }
    
    public static boolean CanSeeBlock(final BlockPos blockPos) {
        return AutoCrystalRewrite.mc.world.rayTraceBlocks(new Vec3d(AutoCrystalRewrite.mc.player.posX, AutoCrystalRewrite.mc.player.posY + AutoCrystalRewrite.mc.player.getEyeHeight(), AutoCrystalRewrite.mc.player.posZ), new Vec3d((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()), false, true, false) == null;
    }
    
    private boolean checkCrystalPlacements(final BlockPos blockPos) {
        return AutoCrystalRewrite.placeWallsRange.getValue() <= 0.0 || CanSeeBlock(blockPos) || blockPos.getDistance((int)AutoCrystalRewrite.mc.player.posX, (int)AutoCrystalRewrite.mc.player.posY, (int)AutoCrystalRewrite.mc.player.posZ) <= AutoCrystalRewrite.placeWallsRange.getValue();
    }
    ...
    public boolean canPlaceCrystal(final BlockPos blockPos) {
        ...
        if (!this.endCrystalMode.getValue()) {
            return (AutoCrystalRewrite.mc.world.getBlockState(blockPos).getBlock() == Blocks.BEDROCK || AutoCrystalRewrite.mc.world.getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN) && AutoCrystalRewrite.mc.world.getBlockState(boost).getBlock() == Blocks.AIR && AutoCrystalRewrite.mc.world.getBlockState(boost2).getBlock() == Blocks.AIR && AutoCrystalRewrite.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost)).isEmpty() && AutoCrystalRewrite.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost2)).isEmpty();
        }
        return (AutoCrystalRewrite.mc.world.getBlockState(blockPos).getBlock() == Blocks.BEDROCK || AutoCrystalRewrite.mc.world.getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN) && AutoCrystalRewrite.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost)).isEmpty() && AutoCrystalRewrite.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost2)).isEmpty();
    }
    
    public static BlockPos getPlayerPos() {
        return new BlockPos(Math.floor(AutoCrystalRewrite.mc.player.posX), Math.floor(AutoCrystalRewrite.mc.player.posY), Math.floor(AutoCrystalRewrite.mc.player.posZ));
    }
    
    private List<BlockPos> findCrystalBlocks() {
        final NonNullList<BlockPos> positions = (NonNullList<BlockPos>)NonNullList.create();
        positions.addAll((Collection)this.getSphere(getPlayerPos(), (float)AutoCrystalRewrite.placeRange.getValue(), (int)AutoCrystalRewrite.placeRange.getValue(), false, true, 0).stream().filter((Predicate<? super Object>)this::canPlaceCrystal).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()));
    ...
    public static float calculateDamage(final double posX, final double posY, final double posZ, final Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = getBlastReduction((EntityLivingBase)entity, getDamageMultiplied(damage), new Explosion((World)AutoCrystalRewrite.mc.world, (Entity)null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float)finald;
    }
    ...
    private static float getDamageMultiplied(final float damage) {
        final int diff = AutoCrystalRewrite.mc.world.getDifficulty().getId();
    ...
    private static void setYawAndPitch(final float yaw1, final float pitch1) {
        AutoCrystalRewrite.yaw = yaw1;
        AutoCrystalRewrite.pitch = pitch1;
        AutoCrystalRewrite.isSpoofingAngles = true;
    }
    
    private static void resetRotation() {
        if (AutoCrystalRewrite.isSpoofingAngles) {
            AutoCrystalRewrite.yaw = AutoCrystalRewrite.mc.player.rotationYaw;
            AutoCrystalRewrite.pitch = AutoCrystalRewrite.mc.player.rotationPitch;
            AutoCrystalRewrite.isSpoofingAngles = false;
        }
    }
    ...
    public void onEnable() {
        ...
        this.isPlacing = false;
        this.isBreaking = false;
        if (this.chat.getValue() && AutoCrystalRewrite.mc.player != null) {
